...
/// <para/>
/// Generally speaking, the ZLib format is merely a DEFLATE stream, prefixed with a header, and performs a cyclic
/// redundancy check to ensure data integrity by storing an Adler-32 checksum after the compressed payload.
/// </summary>
[PublicAPI]
public class ZlibStream : Stream
{
    ...
    public override async ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        adler32.Update(buffer[..read].Span);
        return read;
    }
    ...
    private void ReadCrc()
    {
        ...
        if (crcStream != crcAdler)
            throw new InvalidDataException("CRC checksum validation failed.");
    }
        ...
        switch (compressionMode)
        {
            ...
            case CompressionMode.Decompress:
            {
                if (!IsSupported(BaseStream))
                    throw new InvalidDataException("Unsupported ZLib format.");
